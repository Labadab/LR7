...
public:
    ...
    void Draw(const glm::mat4& view, const glm::mat4& projection, const glm::vec3& viewPos,
        float drumRot, float spindleRot)
    {
        ...
        for (auto& info : meshes) {
            ...
            if (info.name == "Куб.004") {
                glm::vec3 center = info.originalCenter;
                model = glm::translate(model, center);
                model = glm::rotate(model, glm::radians(drumRot), glm::vec3(1.0f, 0.0f, 0.0f));
                model = glm::translate(model, -center);
            }
            else if (info.name == "Куб.001") {
                glm::vec3 center = info.originalCenter;
                model = glm::translate(model, center);
                model = glm::rotate(model, glm::radians(spindleRot), glm::vec3(1.0f, 0.0f, 0.0f));
                model = glm::translate(model, -center);
            }
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(800, 600, "OKNO", NULL, NULL);
    ...
    float drumRot = 0.0f;
    float spindleRot = 0.0f;

    cout << "\n========== УПРАВЛЕНИЕ ==========\n";
    cout << "ВРАЩЕНИЕ БАРАБАНА:    Q / E\n";
    cout << "ВРАЩЕНИЕ ШПИНДЕЛЯ:    R / F\n";
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS) drumRot += rotSpeed;
        if (glfwGetKey(window, GLFW_KEY_E) == GLFW_PRESS) drumRot -= rotSpeed;
        if (glfwGetKey(window, GLFW_KEY_R) == GLFW_PRESS) spindleRot += rotSpeed;
        if (glfwGetKey(window, GLFW_KEY_F) == GLFW_PRESS) spindleRot -= rotSpeed;
        ...
        myModel.Draw(view, projection, cameraPos, drumRot, spindleRot);
